...
for  M = 0:1000:10000
    ...
    BA_throughput_M = [BA_throughput_M sum(x)];
...
for M = 0:1000:10000
    ...
    ER_throughput_M = [ER_throughput_M sum(x)];
...
plot(values_M, ER_throughput_M, 'b-^', 'LineWidth',1,'MarkerSize',8);
hold on;
plot(values_M, BA_throughput_M, 'r--*', 'LineWidth',1,'MarkerSize',10);
set(gca,'xtick',values_M);
xlim([values_M(1), values_M(end)]);
xlabel('Total node Capability M'),ylabel('Throughput'); 
grid on;
