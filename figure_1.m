...
for alpha = [linspace(0.01,0.1,10),linspace(0.1,1,10)]
    ...
    BA_throughput_alpha = [BA_throughput_alpha sum(x)];
...
for alpha = [linspace(0.01,0.1,10),linspace(0.1,1,10)]
    ...
    ER_throughput_alpha = [ER_throughput_alpha sum(x)];
...
plot(values_alpha, BA_throughput_alpha, 'b-^', 'LineWidth',1,'MarkerSize',10);
hold on;
plot(values_alpha, ER_throughput_alpha, 'r--*', 'LineWidth',1,'MarkerSize',8);

set(gca,'xtick',M);
xlim([values_alpha(1), values_alpha(end)]);
xlabel('Parameter \alpha'),ylabel('Throughput'); 
grid on;
